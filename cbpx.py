...
file = open("Stocks/cbpo.us.txt", "r") 
dataset = [[]]
count = 0
for line in file:
    tokens = line.split(',')
...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:2400]
    y_train = dataset_scaled[1:2401]
...
def load():
    test_set = dataset[2201:2300]
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2201:2300]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2390:2400]
    json_file = open('modelTempantm.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    url = ('https://newsapi.org/v2/everything?q=%20cbpx%20stock%20market&apiKey=6e593f373865401e803d6874594f9063')
    ...
    return polarity
def run():
    print('Prediction of cbpx Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
