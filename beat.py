...
file = open("Stocks/beat.us.txt", "r") 
dataset = [[]]
count = 0
for line in file:
    tokens = line.split(',')
...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:1899]
    y_train = dataset_scaled[1:1900]
...
def load():
    test_set = dataset[1900:2000]
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[1900:2000]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2000:2010]
    json_file = open('modelTempantm.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    url = ('https://newsapi.org/v2/everything?q=%20beat%20stock%20market&apiKey=6e593f373865401e803d6874594f9063')
    ...
    return polarity
def run():
    print('Prediction of beat Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
