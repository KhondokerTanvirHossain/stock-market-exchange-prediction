...
dataset = dataset_main.iloc[0:2341, 1:2].values
...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:2000]
    y_train = dataset_scaled[1:2001]
...
def load():
    test_set = dataset_main.iloc[2001:2330, 1:2].values
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2001:2330]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2330:2341]
    json_file = open('modelTemp.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    ...
    return(polarity)
def run():
    print('Prediction of Amazon Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
