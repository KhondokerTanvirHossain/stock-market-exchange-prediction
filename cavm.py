...
file = open("Stocks/cavm.us.txt", "r") 
dataset = [[]]
count = 0
for line in file:
    tokens = line.split(',')
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2500:2510]
    json_file = open('modelTempantm.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    url = ('https://newsapi.org/v2/everything?q=%20bac%20stock%20market&apiKey=6e593f373865401e803d6874594f9063')
    ...
    return polarity
def run():
    print('Prediction of cavm Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
