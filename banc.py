...
file = open("Stocks/banc.us.txt", "r") 
dataset = [[]]
count = 0
for line in file:
    tokens = line.split(',')
...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:1300]
    y_train = dataset_scaled[1:1301]
...
def load():
    test_set = dataset[1301:1400]
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[1301:1400]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[1400:1410]
    json_file = open('modelTempantm.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    url = ('https://newsapi.org/v2/everything?q=%20banc%20stock%20market&apiKey=6e593f373865401e803d6874594f9063')
    ...
    return polarity
def run():
    print('Prediction of banc Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
