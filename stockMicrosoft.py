...
dataset_main = pd.read_csv('msft.csv')
dataset = dataset_main.iloc[0:6993, 1:2].values
...
def load():
    test_set = dataset_main.iloc[6901:6982, 1:2].values
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[6901:6982]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[6982:6993]
    json_file = open('modelTemp.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
 ...
 #   print(polarity)
    return polarity  
def run():
    print('Prediction of Microsoft Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
