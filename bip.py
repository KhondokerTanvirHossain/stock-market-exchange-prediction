...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:2300]
    y_train = dataset_scaled[1:2301]
...
def load():
    test_set = dataset[2301:2400]
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2301:2400]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[2400:2410]
    json_file = open('modelTempantm.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    url = ('https://newsapi.org/v2/everything?q=%20bip%20stock%20market&apiKey=6e593f373865401e803d6874594f9063')
    ...
    return polarity
def run():
    print('Prediction of bip Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
