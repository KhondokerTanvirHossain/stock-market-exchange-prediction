...
def train():    
    ...
    X_train = []
    y_train = []
    X_train = dataset_scaled[0:1000]
    y_train = dataset_scaled[1:1001]
...
def load():
    test_set = dataset_main.iloc[1002:1248, 1:2].values
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[1002:1248]
...
def prediction():
    ...
    #test_set_scaled = sc.transform(test_set)
    test_set_scaled = dataset_scaled[1248:1259]
    json_file = open('modelTemp.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
...
def senti():
    ...
    return(polarity)    
def run():
    print('Prediction of Google Stock Price in Next 10 Days :')
    p =  prediction()
    s = senti()
    print("Date               Price")
    d = 10
